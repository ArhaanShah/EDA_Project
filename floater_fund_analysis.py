# -*- coding: utf-8 -*-
"""Group35_Code (1).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1RgrDN7L3HhQVbeRCW-fCsODPV_deG_Tu

# Environment and Dependencies (Pinned) + Fixed Seed
...
Fixed seed: 42
- Seed applied in Section 1 to `random` and `numpy`.
- For ML models, pass `random_state=42` where applicable.

Optional install (if needed):
```
pip install pandas==1.5.2 numpy==1.23.5 matplotlib==3.6.2 seaborn==0.12.1 scipy==1.9.3 statsmodels==0.13.5 scikit-learn==1.1.3 pmdarima==2.0.3 shap==0.41.0
```
"""
...
# Optional dependencies (documented in the top markdown)
try:
    import pmdarima as pm
except Exception:
    pm = None
    print("Optional: install 'pmdarima==2.0.3' to enable ARIMA search.")

try:
    import shap
except Exception:
    shap = None
    print("Optional: install 'shap==0.41.0' for explainability plots.")
...
print("Setup complete: imports, style, and seeds configured.")
...
if '% Debt Holding' in df_snapshot.columns:
    print("Success: '% Debt Holding' column found.")
else:
...
funds = df_snapshot['Fund Name'].str.split(' - ').str[0] # Shorten names for display
...
returns_melted = returns_df.reset_index().melt(
    'Fund Name', # Corrected from 'index' to 'Fund Name' as it's the actual column name after reset_index()
    var_name='Horizon',
    value_name='Return (%)'
)
...
ax = sns.barplot(
    data=returns_melted,
    x='Fund Name', # Corrected from 'index' to 'Fund Name'
    y='Return (%)',
    hue='Horizon',
    palette='YlGnBu'
)
...
# 3. Prepare Data
fund_col = 'Fund Name' # Define fund_col
...
# Columns: SIP_return_1Y(%), SIP_return_2Y(%), SIP_return_3Y(%), SIP_return_5Y(%) - (These were comments, actual columns are '1Y', '2Y', '3Y', '5Y')
...
# ==========================================
# FIX 1: Create short names to prevent label overlap
# This grabs just the first 2 words of the fund name (e.g., "SBI Floating")
# ==========================================
...
# --- PLOT 1: 1-Year Returns Bar Chart (Top) ---
...
axes[0].set_xlabel('')
axes[0].set_xticklabels([]) # Hide x-axis labels to save space

# --- PLOT 2: Volatility Bar Chart (Middle) ---
...
for label in axes[1].get_xticklabels():
    label.set_ha('right') # Keep them slanted neatly

# --- PLOT 3: Risk-Return Scatter Plot (Bottom) ---
sns.scatterplot(
    data=risk_return_df,
    x='Volatility',
    y='1Y',
...
# ==========================================
# FIX 2: Correct Scatter Plot Annotations
# Calculate a tiny dynamic offset (2% of the y-axis range) instead of +0.1
# ==========================================
...
for i, row in risk_return_df.iterrows():
    axes[2].text(
        ...
        row['Short Name'],
        fontsize=9,
        ha='center' # Centers the short text perfectly over the dot
    )
...
"""### Combined Risk-Return Profiles (1-Year, 3-Year, 5-Year)

This combined plot allows for a direct comparison of each fund's risk-return trade-off across different time horizons. A fund that consistently appears in the top-left quadrant across all three plots (High Return, Low Volatility) is a demonstrably superior long-term, risk-adjusted performer.
"""

# Create short names to prevent label overlap (first 2 words)
risk_return_df['Short Name'] = [' '.join(str(name).split()[:2]) for name in risk_return_df.index]

# Calculate dynamic y-offsets for each specific plot (2% of the total y-axis range)
def get_offset(col_name):
    y_range = risk_return_df[col_name].max() - risk_return_df[col_name].min()
    return y_range * 0.02 if y_range > 0 else 0.001
...
# Ensure short names are created to prevent long overlapping labels
if 'Short Name' not in risk_return_df.columns:
    risk_return_df['Short Name'] = [' '.join(str(name).split()[:2]) for name in risk_return_df.index]
...
ax = sns.scatterplot(
    ...
    x='Volatility',
    y='3Y',
    hue='Short Name', # Using Short Name
    s=200,
    palette='tab10',
    legend=False
)
...
# Calculate a dynamic offset for the 3-Year data (2% of the total range)
...
for i, row in risk_return_df.iterrows():
    ax.text(
        row['Volatility'],
        row['3Y'] + y_offset, # Use dynamic offset instead of +0.1
        row['Short Name'],    # Use short name instead of full split
        fontsize=9,
        ha='center'           # Center the text directly above the dot
    )
...
Section 4: Additional Risk & Portfolio Analytics
- Efficient Frontier (Monte Carlo)
- ML Explainability for return drivers (SHAP)
These belong to risk/portfolio analytics and are grouped here, not in time-series.
"""

# --- CELL 1: Return Distribution & Shape Analysis ---

# Ensure pivot_change is defined
# 1. Setup Data for pivot_change
# Use 'Fund Name' as it is the standard in your current dataframe
fund_col = 'Fund Name'
pivot_nav = df.pivot_table(index='Date', columns=fund_col, values='NAV')
pivot_nav = pivot_nav.ffill().dropna()

# 2. Calculate Daily Returns (pivot_change)
...
# OPTIMIZATION 1: Drop potential intraday duplicates, then use pivot() instead of pivot_table().
# pivot() is dramatically faster because it does not attempt to calculate aggregations.
...
# OPTIMIZATION 2: Fully vectorized CVaR calculation.
# This creates a boolean mask of the entire dataframe instantly, entirely avoiding the slow .apply() lambda.
...
# Sort by Max Drawdown (Safest funds on top)
...
# Columns: Standard Deviation, SIP_return_3Y(%)

plt.figure(figsize=(10, 6))
# Fixed column names: 'Standard Deviation', 'Fund Name', and '3Y'
sns.scatterplot(data=df_snapshot, x='Standard Deviation', y='3Y',
                size='AUM (Crore)', sizes=(100, 1000), hue='Fund Name', alpha=0.7)

# Label points
for i in range(df_snapshot.shape[0]):
    # Using iloc and correct column names to avoid indexing errors
    plt.text(df_snapshot['Standard Deviation'].iloc[i]+0.001, df_snapshot['3Y'].iloc[i],
             df_snapshot['Fund Name'].iloc[i].split()[0], fontsize=9)
...
# --- Sharpe Ratio Plot ---
# OPTIMIZATION 2: Add errorbar=None to prevent Seaborn from running slow statistical bootstrapping.
sns.barplot(
    ax=axes[0],
    data=df_sharpe_sorted,
    y='Fund Name',
    x='Sharpe Ratio',
    palette='magma',
    errorbar=None
...
# Print statements using our pre-sorted dataframes
print("\n--- Risk Metric Leaders ---")
print(f"Best Risk-Adjusted Return (Sharpe): {df_sharpe_sorted.iloc[0]['Fund Name']}")
print(f"Highest Alpha Generator: {df_alpha_sorted.iloc[0]['Fund Name']}")
...
sns.barplot(
    ax=axes[1],
    data=df_snapshot.sort_values("Treynor's Ratio", ascending=False), # Sorted by Best Performance
    y='Fund Name',
    x="Treynor's Ratio",
    palette='viridis'
)
...
print(f"Least Volatile Fund (Lowest Beta): {lowest_beta['Fund Name']} ({lowest_beta['Beta']})")

print("\n--- Efficiency Analysis ---")
best_treynor = df_snapshot.sort_values("Treynor's Ratio", ascending=False).iloc[0]
print(f"Best Risk-Adjusted Return (Treynor): {best_treynor['Fund Name']} ({best_treynor['Treynor\'s Ratio']})")

"""## Sharpe Ratio Bar Chart with Rolling Error Bars
We estimate annualized Sharpe = (annualized return − risk-free rate) / annualized volatility.
Then, we add error bars from rolling-window Sharpe to visualize stability across time.
"""

# Risk/Portfolio Analytics: Sharpe Ratio
# Inputs: df_snapshot with columns: 'Fund_Name', 'Standard_Deviation' (daily), and periodic returns like '1Y'
# Assumptions: We estimate annualized return using 1Y (%), and annualized vol from daily std * sqrt(252).
risk_free_rate = 0.05  # Example 5% annual RF; adjust as needed
...
# Rolling-window Sharpe (using df time series) for error bars
...
# Map rolling Sharpe std as error bars
err_bars = rolling_sharpe.std().reindex(df_sharpe['Fund Name']).values

# Plot
plt.figure(figsize=(10, 6))
ax = sns.barplot(data=df_sharpe.sort_values('Sharpe', ascending=False),
                  y='Fund Name', x='Sharpe', palette='magma')
# Add error bars
ax.errorbar(df_sharpe.sort_values('Sharpe', ascending=False)['Sharpe'],
            range(len(df_sharpe)),
            xerr=err_bars[df_sharpe.sort_values('Sharpe', ascending=False).index],
...
"""## Treynor vs Beta (Bubble Plot)
Treynor Ratio measures excess return per unit of systematic risk (Beta).
Bubble size encodes AUM or Sharpe to convey scale or efficiency.
"""

# Risk/Portfolio Analytics: Treynor vs Beta Bubble
# Inputs: df_snapshot columns include 'Treynor_Ratio', 'Beta', 'AUM (Crore)', optional 'Sharpe_Ratio'
...
plt.figure(figsize=(10, 6))
sns.scatterplot(data=df_snapshot, x='Beta', y='Treynor\'s Ratio',
                size=sizes if sizes is not None else None, sizes=(100, 1000),
                hue='Fund Name', alpha=0.7)

for i in range(df_snapshot.shape[0]):
    plt.text(df_snapshot['Beta'].iloc[i]+0.01, df_snapshot['Treynor\'s Ratio'].iloc[i],
             df_snapshot['Fund Name'].iloc[i].split()[0], fontsize=9)
...
"""## Rolling Mean & Rolling Volatility (Twin Axes)
For each fund, we plot rolling 30-day mean return and rolling 90-day volatility to visualize stability and risk.
Use twin y-axes to show both measures on one figure per fund.
"""

# Risk/Portfolio Analytics: Rolling Mean & Volatility
fund_col = 'Fund_Name' if 'Fund_Name' in df.columns else 'Fund Name'
pivot_nav = df.pivot_table(index='Date', columns=fund_col, values='NAV').sort_index().ffill()
daily_ret = pivot_nav.pct_change()
...
for fund in daily_ret.columns:
    ...
    rolling_mean = series.rolling(mean_window).mean() * 252  # annualize
...
Heatmap shows time (x) vs fund (y) colored by rolling Sharpe (e.g., 90-day).
This offers a quick visual for consistency over time.
"""

# Risk/Portfolio Analytics: Rolling Sharpe Heatmap
fund_col = 'Fund_Name' if 'Fund_Name' in df.columns else 'Fund Name'
pivot_nav = df.pivot_table(index='Date', columns=fund_col, values='NAV').sort_index().ffill()
daily_ret = pivot_nav.pct_change()
...
# Convert to matrix suitable for heatmap by resampling weekly
roll_sharpe_weekly = roll_sharpe.resample('W').mean()

# ==========================================
# FIX: Drop the initial NaN values caused by the rolling window
# ==========================================
roll_sharpe_weekly = roll_sharpe_weekly.dropna(how='all')

# Format the datetime index to a clean string
roll_sharpe_weekly.index = roll_sharpe_weekly.index.strftime('%Y-%m-%d')

plt.figure(figsize=(12, 6))

# Plot heatmap, skipping labels to prevent crowding
sns.heatmap(
    roll_sharpe_weekly.T,
    cmap='YlGnBu',
    center=0,
    xticklabels=13,
    cbar_kws={'label': 'Rolling Sharpe (90d, annualized)'}
)
...
"""## Max Drawdown: Definition & Importance
Max Drawdown captures the largest peak-to-trough decline.
It highlights worst-case losses and complements volatility measures.
"""

# Risk/Portfolio Analytics: Drawdown Charts
fund_col = 'Fund_Name' if 'Fund_Name' in df.columns else 'Fund Name'
pivot_nav = df.pivot_table(index='Date', columns=fund_col, values='NAV').sort_index().ffill()

# Drawdown series per fund
rolling_max = pivot_nav.cummax()
drawdown = (pivot_nav - rolling_max) / rolling_max

# Bar chart of Max Drawdown per fund
...
# Time-series drawdown plots with worst periods highlighted
...
"""## Time to Recovery: Why It Matters
Time-to-recovery measures how quickly a fund rebounds from drawdowns.
Debt funds typically have modest drawdowns; recovery speed is informative for risk.
"""

# Risk/Portfolio Analytics: Time to Recovery
fund_col = 'Fund_Name' if 'Fund_Name' in df.columns else 'Fund Name'
pivot_nav = df.pivot_table(index='Date', columns=fund_col, values='NAV').sort_index().ffill()

# FIX 1: Ensure the index is a datetime object so date math (.days) works correctly
pivot_nav.index = pd.to_datetime(pivot_nav.index)

# Compute time to recovery: max days from a peak to the next peak
recovery_days = {}
for fund in pivot_nav.columns:
    ...
    max_recovery = 0
    last_peak_date = s.index[0] # Initialize with the first available date

    for dt, val in s.items():
        # FIX 2: If current value matches the historical max, we hit a peak (or fully recovered)
        if val >= cummax.loc[dt]:
            days_to_recover = (dt - last_peak_date).days
            max_recovery = max(max_recovery, days_to_recover)
            last_peak_date = dt # Reset the peak date marker

    # FIX 3: Check if the fund is currently in an unrecovered drawdown at the very end of the dataset
...
rec_df = pd.Series(recovery_days).sort_values(ascending=True) # Ascending looks better on horizontal bars
...
plt.ylabel('')
plt.grid(axis='x', linestyle='--', alpha=0.7) # Added x-grid for better readability
plt.tight_layout()
plt.show()

"""# ***Section 5: Part 4 - Time Series Analysis & Forecasting (NAV)***

This section focuses exclusively on time-series modeling: trend extraction, lag features, ARIMA variants, and forecasting plots for NAV series.

## STL Decomposition: Purpose
Seasonal-Trend decomposition using LOESS (STL) separates NAV into trend, seasonal, and residual.
This reveals month-of-year seasonality or systematic patterns.
"""

# Time-Series: STL Decomposition
from statsmodels.tsa.seasonal import STL

# Choose representative fund and build regular series
fund_col = 'Fund_Name' if 'Fund_Name' in df.columns else 'Fund Name'
fund_sel = df[fund_col].unique()[0]
ts_nav = df[df[fund_col] == fund_sel].set_index('Date')['NAV'].sort_index().asfreq('D').ffill()

# STL on NAV
stl = STL(ts_nav, period=30)  # approx monthly
res = stl.fit()
...
# Also do STL on monthly returns to reveal month-of-year effects
monthly_ret = ts_nav.pct_change().resample('M').sum().dropna()
...
This subsection replicates the time-series methodology (trend + lags + ARIMA) for other funds. The same NAV-focused approach is applied per fund.
"""
...
# Make sure to import pmdarima to prevent the NoneType error
import pmdarima as pm

# 1. Split your prepared data ('ts') into Training and Testing sets
# We use 80% of the data to train the model, and hold back 20% to test its accuracy later
train_size = int(len(ts) * 0.8)
train = ts.iloc[:train_size]
test = ts.iloc[train_size:]

print(f"Total days: {len(ts)} | Training on: {len(train)} days | Testing on: {len(test)} days\n")
...
# ==========================================
# FIX: Added trend='t' to the ARIMA model.
# This forces the model to calculate a linear deterministic trend
# based on historical data and apply it to the future forecast.
# ==========================================
